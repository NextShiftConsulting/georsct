"""Tree-based kappa gate for S_sup detection."""
...
class KappaGate:
    """LightGBM-based gate for detecting superfluous (S_sup) content.
    ...
    @classmethod
    def load(cls, path: str) -> "KappaGate":
        ...
        if isinstance(data, dict):
            gate.model = data["model"]
            gate.threshold = data.get("threshold", 0.5)
        else:
            gate.model = data
    ...
    def train(
        ...
        **lgb_params
        ...
        gate = KappaGate(model=LGBWrapper(model))
        return gate
