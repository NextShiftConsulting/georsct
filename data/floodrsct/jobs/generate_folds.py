...
def assign_spatial_blocked_folds(
    ...
    Returns (zcta->fold dict, blocking_strategy used).
    ...
    if n_counties >= n_folds:
        ...
        if n_zip3 < n_folds:
            log.warning(
                "ZIP3 groups (%d) still fewer than folds (%d). "
                "Some folds will be empty.", n_zip3, n_folds,
            )
    ...
    # Bin-pack blocks into folds
    block_fold = _greedy_bin_pack(block_sizes, n_folds, seed)

    # Map to ZCTAs
    assignments = {}
    for block, fold in block_fold.items():
        for z in block_zctas[block]:
            assignments[z] = fold
    ...
    for fold in assignments.values():
        fold_sizes[fold] += 1
    log.info("Spatial-blocked (%s) fold sizes: %s", strategy, fold_sizes)
...
def generate_folds(
    ...
    # 2. Spatial-blocked CV
    blocked_map, block_strategy = assign_spatial_blocked_folds(
