#!/usr/bin/env python3
"""
SageMaker Launcher: Drive-Time Feature Build

Launches a processing job to compute per-ZCTA drive-time features using
HIFLD 2022 hospital locations and ZCTA centroids.

Two features:
  - drive_min_to_nearest_hospital: haversine-estimated drive time (min)
  - drive_min_to_county_centroid: haversine-estimated drive time (min)

Instance: ml.m5.xlarge (4 vCPU, 16 GB RAM, $0.23/hr)
Runtime:  25-35 min estimated (OSRM table API: ~1,300 batched calls x 2 features)
Cost:     ~$0.13 estimated

Mounts:
  Code:   s3://swarm-yrsn-datasets/rsct_code/geocert_v24/drive_times/
...
Usage:
  python sagemaker_drive_times.py --dry-run
  python sagemaker_drive_times.py
"""
...
CODE_PREFIX = "rsct_code/geocert_v24/drive_times"
...
CODE_FILES = [
    "build_drive_times.py",
    "build_hifld_features.py",  # imported by build_drive_times for load_hifld_hospitals()
    "entrypoint_drive_times.sh",
]
...
def main():
    parser = argparse.ArgumentParser(description="Launch drive-time SageMaker job")
    ...
    # Verify ZCTA data
    zcta_key = f"{DATA_PREFIX}/zcta_features_labels.parquet"
    try:
        s3.head_object(Bucket=BUCKET, Key=zcta_key)
        print(f"  ZCTA data verified: s3://{BUCKET}/{zcta_key}")
    except Exception:
        print(f"  ERROR: ZCTA data not found at s3://{BUCKET}/{zcta_key}")
        sys.exit(1)

    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    job_name = f"geocert-drive-times-{timestamp}"
    image_uri = get_image_uri(REGION)

    print("\n" + "=" * 60)
    print("Drive-Time Feature Build")
    print("=" * 60)
    ...
    if args.dry_run:
        ...
        print(f"[DRY RUN] Estimated runtime: 25-35 min (OSRM table API)")
        print(f"[DRY RUN] Estimated cost: ~$0.13 ({args.instance_type} @ $0.23/hr)")
        return
    ...
    config = {
        ...
        "AppSpecification": {
            ...
            "ContainerEntrypoint": [
                "bash",
                "/opt/ml/processing/input/code/entrypoint_drive_times.sh",
            ],
        },
        "RoleArn": role_arn,
        "ProcessingInputs": [
            ...
            },
            {
                "InputName": "data",
                "S3Input": {
                    "S3Uri": f"s3://{BUCKET}/rsct_code/geocert_v24/inputs/",
