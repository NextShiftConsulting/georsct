...
Cross-validate GeoRSCT's 21 CDC PLACES health targets against the official
...
| Set | Count |
|-----|-------|
| GeoRSCT ZCTAs | 31,789 |
| HHS PLACES 2023 GIS ZCTAs | 32,409 |
| In both | 31,529 |
| GeoRSCT-only | 260 |
| HHS-only | 880 |

**260 GeoRSCT-only ZCTAs:** These are exactly the ZCTAs flagged `has_cdc_places=False`
in GeoRSCT. They have ACS features and non-CDC targets (elevation, tree cover, etc.)
...
from GeoRSCT. Our canonical ZCTA set (31,789) is defined by the intersection of
...
Of those, 969 overlap with GeoRSCT ZCTAs where we DO have CDC PLACES data
(`has_cdc_places=True`).

This means GeoRSCT's PLACES source has broader coverage than the HHS HF
...
**This is not a data integrity issue.** GeoRSCT has MORE CDC PLACES coverage
than the HHS GIS-friendly release, not less. For every ZCTA where both datasets
have values, the values are identical.

## Column Name Mapping

For reproducibility, the mapping between GeoRSCT target names and official
