...
Computes 95% CIs on TRF per task via fold-level block bootstrap (B=1000).
...
Bootstrap method:
  ...
  TRF = 1 - max_k(mean_R2_k) over the resampled fold set.
  B=1000 iterations, 2.5th/97.5th percentile CIs.

Cross-seed consistency:
  Loads seed_42, seed_123, seed_456 results.
  Reports per-task TRF mean and std across seeds.
...
Output:
  s019d_bootstrap_cis.json   -- per-task TRF CIs + cross-seed + kappa ρ CIs
...
RESULTS_PREFIX = "rsct_curriculum/series_019/results/s019d"
SEEDS = [42, 123, 456]
EMBEDDINGS = [
...
def compute_task_residual_floor(r2_matrix, task):
    """TRF = 1 - max_k(mean R2_k) for one task."""
    emb_means = []
...
def fold_block_bootstrap_task_residual_floor(r2_matrix, task, n_boot=N_BOOT, seed=0):
    """
    Fold-level block bootstrap for TRF of one task.

    For each bootstrap replicate:
      - Resample 5 folds with replacement (5->5)
      - Per embedding: mean R2 over resampled folds
      - TRF = 1 - max_k(mean R2_k)
...
def main():
    ...
    # --- Per-task TRF bootstrap (primary seed) ---
    log.info("\nFold-level block bootstrap, B=%d...", args.n_boot)
    per_task_bootstrap = {}
    for task in all_tasks:
        point, ci_low, ci_high, boot_samples = fold_block_bootstrap_task_residual_floor(
        ...
        per_task_bootstrap[task] = {
            "task_residual_floor": round(point, 4) if np.isfinite(point) else None,
    ...
    for task in all_tasks:
        ...
        for seed, records in seed_records.items():
            ...
            nc = compute_task_residual_floor(matrix, task)
    ...
    if cross_seed_stds:
        log.info("  Cross-seed TRF std: mean=%.4f max=%.4f",
    ...
    task_residual_floor_vals = [v["task_residual_floor"] for v in per_task_bootstrap.values() if v["task_residual_floor"] is not None]

    log.info("\n" + "=" * 60)
    log.info("BOOTSTRAP CI SUMMARY (primary seed=%d, B=%d)", PRIMARY_SEED, args.n_boot)
    log.info("  TRF range: %.4f - %.4f", min(task_residual_floor_vals), max(task_residual_floor_vals))
    ...
    output = {
        "experiment": "S019D",
        "description": "Fold-level block bootstrap CIs on TRF (B=1000, 6-family)",
        ...
        "task_residual_floor_summary": {
            "min": round(min(task_residual_floor_vals), 4),
            "max": round(max(task_residual_floor_vals), 4),
            "mean": round(float(np.mean(task_residual_floor_vals)), 4),
            "range_ratio": round(max(task_residual_floor_vals) / min(task_residual_floor_vals), 1) if min(task_residual_floor_vals) > 0 else None,
        },
        "ci_width_summary": {
    ...
    fieldnames = [
        "task", "family", "task_residual_floor", "ci_low", "ci_high", "ci_width",
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for task in all_tasks:
            ...
            writer.writerow({
                "task": task,
                "family": bt["family"],
                "task_residual_floor": bt["task_residual_floor"],
                "ci_low": bt["ci_low"],
                "ci_high": bt["ci_high"],
    ...
    summary = {
        ...
        "primary_seed": PRIMARY_SEED,
        "n_boot": args.n_boot,
        "task_residual_floor_range": [
            round(min(task_residual_floor_vals), 4),
            round(max(task_residual_floor_vals), 4),
        ],
        "task_residual_floor_range_ratio": output["task_residual_floor_summary"]["range_ratio"],
    ...
    log.info("TRF: %.4f - %.4f (%.1fx range)",
             min(task_residual_floor_vals), max(task_residual_floor_vals),
             output["task_residual_floor_summary"]["range_ratio"])
