...
def main():
    ...
    # Classify columns
    meta_cols = ["zcta_id", "county_fips", "county_name", "state_fips", "state",
                 "latitude", "longitude"]
    split_cols = ["split_imputation", "split_extrapolation", "split_superres"]
    coverage_cols = ["has_cdc_places", "has_income", "has_home_value", "has_cdc_ci"]
    acs_cols = sorted(c for c in geo.columns if c.startswith("acs_"))
    target_cols = sorted(
        c for c in geo.columns
        if c.startswith("target_") and not c.endswith("_ci_low") and not c.endswith("_ci_high")
    )
    ci_cols = sorted(c for c in geo.columns if c.endswith("_ci_low") or c.endswith("_ci_high"))
    svi_cols = sorted(c for c in geo.columns if c.startswith("svi_"))
    flood_cols = sorted(c for c in geo.columns if c.startswith("flood_"))
    hifld_cols = sorted(c for c in geo.columns if c.startswith("hifld_"))
    drive_cols = sorted(c for c in geo.columns if c.startswith("drive_"))
    ...
    for col in geo.columns:
        if col == "geometry":
            ...
            if col in meta_cols:
                ...
                info["group"] = "target_ci"
                info["source"] = "CDC PLACES 2023 95% CI"
                info["min"] = round(float(series.min(skipna=True)), 4)
                info["max"] = round(float(series.max(skipna=True)), 4)
            elif col in svi_cols:
                info["group"] = "feature"
                info["source"] = "CDC/ATSDR SVI 2022"
                info["min"] = round(float(series.min(skipna=True)), 4)
                info["max"] = round(float(series.max(skipna=True)), 4)
                info["mean"] = round(float(series.mean(skipna=True)), 4)
            elif col in flood_cols:
                info["group"] = "feature"
                info["source"] = "FEMA NFHL"
                info["min"] = round(float(series.min(skipna=True)), 4)
                info["max"] = round(float(series.max(skipna=True)), 4)
                info["mean"] = round(float(series.mean(skipna=True)), 4)
            elif col in hifld_cols:
                info["group"] = "feature"
                info["source"] = "DHS HIFLD Open Data"
                info["min"] = round(float(series.min(skipna=True)), 4)
                info["max"] = round(float(series.max(skipna=True)), 4)
                info["mean"] = round(float(series.mean(skipna=True)), 4)
            elif col in drive_cols:
                info["group"] = "feature"
                info["source"] = "OSRM / haversine estimate"
                info["min"] = round(float(series.min(skipna=True)), 4)
                info["max"] = round(float(series.max(skipna=True)), 4)
                info["mean"] = round(float(series.mean(skipna=True)), 4)
    ...
    manifest = {
        ...
        "description": "27 geospatial regression tasks across 31,789 US ZCTAs with uncertainty, vulnerability, and infrastructure layers",
        "sources": {
            ...
            "cdc_places_ci": "https://data.cdc.gov/resource/c76y-7pzg.json",
            "cdc_svi": "https://svi.cdc.gov/Documents/Data/2022/csv/states/SVI_2022_US.csv",
            "fema_nfhl": "https://hazards.fema.gov/gis/nfhl/rest/services/public/NFHL/MapServer",
            "hifld_hospitals": "https://hifld-geoplatform.opendata.arcgis.com/",
            "census_tract_zcta_xwalk": "https://www2.census.gov/geo/docs/maps-data/data/rel2020/zcta520/tab20_zcta520_tract20_natl.txt",
        },
        "geometry": {
        ...
        "stats": {
            ...
            "n_target_ci": len(ci_cols),
            "n_svi": len(svi_cols),
            "n_flood": len(flood_cols),
            "n_hifld": len(hifld_cols),
            "n_drive": len(drive_cols),
        ...
        "licensing": {
            ...
            "CDC_SVI": "Public domain (US government work)",
            ...
            "FEMA_NFHL": "Public domain (US government work)",
