#!/usr/bin/env python3
"""
load_georsct.py -- Load and validate GeoRSCT benchmark data.
...
Copy this file alongside the parquet files. It handles:
  ...
  - Separating features (acs_*, lag_acs_*, svi_*, flood_*, hifld_*, drive_min_*) from targets (target_*)
  - Filtering by coverage flags (has_cdc_places, has_income, has_home_value)
  - Basic data validation

Usage:
    from load_georsct import load_geocert, get_split
...
def load_georsct(path: str) -> pd.DataFrame:
    """Load GeoRSCT parquet (table or geoparquet).

    Args:
        path: Path to georsct_table.parquet or georsct_simplified_001.geoparquet
...
_FEATURE_PREFIXES = ("acs_", "lag_acs_", "svi_", "flood_", "hifld_", "drive_min_")


def feature_columns(df: pd.DataFrame) -> list:
    """Return all 63 solver-usable feature column names.

    Includes ACS (33), spatial-lag (14), and geospatial enrichment (16) features.
    For an ACS-only baseline, filter to columns starting with 'acs_' only.
    """
    return sorted(
        c for c in df.columns if c.startswith(_FEATURE_PREFIXES)
    )
...
if __name__ == "__main__":
    ...
    path = sys.argv[1] if len(sys.argv) > 1 else "georsct_table.parquet"
    df = load_georsct(path)

    print(f"Loaded: {len(df)} ZCTAs, {len(df.columns)} columns")
    print(f"Features: {len(feature_columns(df))} solver-usable columns")
