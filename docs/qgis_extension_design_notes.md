...
| **Zonal Exact Extract / Zonify** | (aggregation) | Phase 7 (FAST) | Partial pixel coverage weighting for zonal statistics. Percentile statistics (P10, P50, P90), not just mean/sum. Batch processing across return periods. | ZCTA aggregation (see `georsct/evaluation/aggregate.py`). **Gap: add depth percentiles to FAST ZCTA features. Use exact-extract weighting for boundary ZCTAs.** |
