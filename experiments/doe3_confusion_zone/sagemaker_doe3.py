"""sagemaker_doe3.py - Launch DOE-3 confusion zone analysis."""
import boto3
import sagemaker
...
CODE_S3_PATH = "s3://yrsn-datasets/rsct_code/doe3_confusion_zone/run_doe3.py"


def launch_job(dry_run: bool = False):
    session = sagemaker.Session(boto3.Session(region_name=REGION))
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    job_name = f"doe3-confusion-{timestamp}"
    ...
    processor.run(
        ...
        outputs=[ProcessingOutput(
            ...
            destination=f"s3://{BUCKET}/s017/doe3_confusion_zone/results/{timestamp}/",
        )],
