...
def _load_latents(art_path: Path, df: pd.DataFrame) -> np.ndarray:
    """Load a _latents.npz artifact with zcta_id alignment.

    GNN latent arrays must be reindexed to match df's row order.
    Without this, misaligned ZCTA ordering produces silently wrong
    embeddings (R2 ~ 0). See run_s019a.py lines 685-736 for the
    original fix; ported here so all callers are safe.
    """
    art = np.load(art_path, allow_pickle=True)
    ...
    # --- GNN zcta_id alignment (Fix 1, ported from run_s019a.py) ---
    if "zcta_id" not in art:
        raise KeyError(
            f"{art_path.name}: missing canonical 'zcta_id' key; "
    ...
    missing_ids = [zid for zid in df_ids if zid not in gnn_id_to_idx]
    if missing_ids:
        raise KeyError(
            f"{art_path.name}: {len(missing_ids)} df ZCTAs not in artifact "
            f"(first 5: {missing_ids[:5]})"
        )

    reindex = np.array([gnn_id_to_idx[zid] for zid in df_ids])
    already_aligned = int(np.sum(reindex == np.arange(len(reindex))))
    Z = Z_raw[reindex]

    log.info(
        "%s: loaded latents (shape %s), reindexed by zcta_id (%d/%d already in order)",
        art_path.name, Z.shape, already_aligned, len(reindex),
    )
    return Z
            ...
            for artifact_map in (REPR_ARTIFACTS, REPR_ARTIFACTS_LEGACY):
                ...
                if filename and (repr_dir / filename).exists():
                    ...
                    if "_transform" in filename or filename in (
                        ...
                        embeddings[emb_name] = _load_latents(path, df)
                    loaded = True
                    break
