#!/usr/bin/env python3
"""launch_hazard_shape.py -- DOE-H1: Hazard Shape ablation test.

3-arm ablation (depth_scalar / depth_shape / depth_vector) per scenario.
Tests whether multi-return-period Deltares depths outperform a single
tail-risk scalar.
...
Deployment Resource Review (9 dimensions):
  1. Memory:    3 x HGB fits per fold x ~5 folds. Each HGB with max_bins=32
                ...
                data loading + join + Deltares cache: ~500 MB. Bootstrap CI:
                negligible. Budget: ~1.5 GB peak per scenario. 8 GB sufficient.
  2. Cache:     Reads processed/shared/zcta_deltares_depth.parquet (shared).
                No cache writes. Reads folds/{scenario}_folds.parquet.
  3. Threads:   HGB uses all cores internally. 3 sequential arms, each with
                sequential folds. 2 vCPU sufficient.
...
def _launch_one(scenario: str, dry_run: bool) -> str:
    job_name = make_job_name(f"doeh1-{scenario[:8].replace('_', '-')}")
    return launch_processing_job(
        job_name=job_name,
        job_script="run_hazard_shape.py",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch DOE-H1 hazard shape ablation (SageMaker)"
    )
