...
| **ScenarioRegistry** | `georsct/db/scripts/scenario_registry.py` | Already a port pattern: `ScenarioRegistry.from_db()` and `from_schema_dir()` with dual lookup (by pipeline_name and scenario_id). Maps to `ports/scenario_registry.py`. |
| **Database schema** | `georsct/db/schema/001_scenarios.sql`, `006_scenario_pipeline.sql` | Adapter implementation: PostGIS/DuckDB tables backing the ScenarioRegistry port. Maps to `adapters/outbound/postgis/`. |
